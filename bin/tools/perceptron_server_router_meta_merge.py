...
The tool can be used via the following command
	python3 path/to/this/file path/to/benchmark/directory number_of_helper_gen_processes_to_use number_of_helper_processes_to_use_for_other_tasks
...
def predict(bias, weights, vector):
	used_idx = [0,1,4] # Use only buffer status, cycles since last flit, transmitted flits and buffer waiting time
...
def main():
	...
	os.system("touch " + dir_base_name + "/accuracy_report")
	os.system("touch " + dir_base_name + "/weights")
