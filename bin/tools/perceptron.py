...
The tool can be used via the following command
	python3 path/to/this/file path/to/features/file
"""
...
DIM_X = 8
DIM_Y = 8
# Note that the grid has origin at the top left corner and the coordinates increse in bottom right direction
...
def get_connected_router_and_port(router_port):
	...
	if(router_port[1] == DIRECTION_NORTH):
		other_router = list(router_port[0])
		other_router[1] -= 1 
...
PORT_FEATURE_OFFSET = 3 # The index (0 based) of first port feature
...
def parse_features(feature_file, router_and_ports):
	# # Initialise router_info. This is done so that later this can be used as hash set to query whether a router and port pair is required
	# router_info = {}
	# for router_port in router_and_ports:
	# 	router_info[router_port] = []
...
# definition for nomalization value
NORMALIZATION_VALUE = 10
"""
Normalizes every feature between 0 and NORMALIZATION_VALUE
Args:
	router_info	: Input data (Not preserved)
Rets:
	router_info	: Updated values
"""
def normalize_data(router_info):
	print("Normalizing...")
	for router_port in router_info:
		# Initialize values
		max_vals = {	# Initialize max values
			PARSED_BUFFER_STATUS 			: -1e5,
			PARSED_CYCLES_SINCE_LAST_FLIT	: -1e5,
			PARSED_STALLED_FLITS			: -1e5,
			PARSED_TRANSMITTED_FLITS		: -1e5,
			PARSED_BUFFER_WAITING_TIME		: -1e5
		}
		min_vals = {	# Initialize min values
			PARSED_BUFFER_STATUS 			: 1e5,
			PARSED_CYCLES_SINCE_LAST_FLIT	: 1e5,
			PARSED_STALLED_FLITS			: 1e5,
			PARSED_TRANSMITTED_FLITS		: 1e5,
		...
		for entry_idx in range(len(router_info[router_port])):
			...
			for feature in max_vals:
				max_vals[feature] = max(router_info[router_port][entry_idx][feature], max_vals[feature])
				min_vals[feature] = min(router_info[router_port][entry_idx][feature], min_vals[feature])

		# Map value via following equation: new_val = (old_val - min_val) * NORMALIZATION_VALUE / (max_val - min_val)
		for entry_idx in range(len(router_info[router_port])):
			for feature in max_vals:
				old_value = router_info[router_port][entry_idx][feature]	# Fetch old value
				new_value = -1
				if(max_vals[feature] == min_vals[feature]):	# Check if feature never changed
					new_value = 0	# Set as zero as this has no effect on perceptron
				else:
					new_value = (old_value - min_vals[feature]) * (NORMALIZATION_VALUE / (max_vals[feature] - min_vals[feature]))	# Calculate scaled value
				router_info[router_port][entry_idx][feature] = new_value # Update to new value
	print("Done!")
	return router_info
...
TRAINING_REPORT = "training_report"	# Name of the file if we wish to export data
"""
...
"""
def train_and_test(test, train, router_port):
...
"""
def run_experiment(router_info):
	...
	for router_port in router_info: # Run training and testing for each router and port
		...
		cur_accuracy = train_and_test(test, train, router_port)
...
Rets:
	...
	start_cycle	: Cycle at which the attack started
"""
def get_annotation_data():
	...
	print("Starting cycle : ", end = "")
	start_cycle = int(input())
	print("Done!")
	return router_1, router_2, start_cycle
...
def rec_path(router_and_ports, current, destination, direction):
	...
	if(destination[1] > current[1]):
		current[1] += 1
		rec_path(router_and_ports, current, destination, DIRECTION_NORTH)
		return

	if(destination[1] < current[1]):
		current[1] -= 1
		rec_path(router_and_ports, current, destination, DIRECTION_SOUTH)
		return

	if(destination[1] == current[1]):
		dir_next = DIRECTION_WEST if (destination[0] > current[0]) else DIRECTION_EAST
		current[0] += 1 if (destination[0] > current[0]) else -1
		rec_path(router_and_ports, current, destination, dir_next)
		return
...
def generate_path(router_1, router_2):
	if(router_1 == router_2):
		return None
	forward_path = []
	rec_path(forward_path, cp(router_1), cp(router_2), -1)	# Generate first part of path
	backward_path = []
	rec_path(backward_path, cp(router_2), cp(router_1), -1) # Generate second part of path
	path = forward_path[1:] + backward_path[1:]	# Join both parts to complete the cycle and return. The first router in both paths is ignored as it's port is unknown
