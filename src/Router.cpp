		...
		for (int i = 0; i < DIRECTIONS + 2; i++) 
		{
			...
			{ 
				...
				if (!buffer[i][vc].IsFull()) 
				{
					...
					received_flit.rx_cycle = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;	// Store the cycle at which flit was received
...
void Router::txProcess()
{
	...
	{ 
		current_features.reset_counts_for_tx();	// Reset counters for features		
		...
		current_features.routed_flits = routed_flits;	// Update routed flits
	}   
}
